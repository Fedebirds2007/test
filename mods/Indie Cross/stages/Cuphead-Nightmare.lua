...
function onUpdate()
	if getProperty('dad.curCharacter') ~= 'Nightmare-Cuphead' then
		setProperty('dad.color',getColorFromHex('FF6696'))
	end
	if getProperty('boyfriend.curCharacter') ~= 'bf-cuphead-nm' then
		setProperty('boyfriend.color',getColorFromHex('FF6696'))
	end
end